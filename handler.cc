...
static inline float myabs(float x) { return x > 0 ? x : -x; }
...
public:
	virtual void raw_motion(RTriple e, bool abs_x, bool abs_y) {
		...
		if (myabs(offset_x) > 1.0) {
			n1 = (int)floor(myabs(offset_x));
			if (offset_x > 0) {
				b1 = 7;
				offset_x -= n1;
			} else {
		...
		if (myabs(offset_y) > 1.0) {
			if (myabs(offset_y) < 1.0)
				return;
			n2 = (int)floor(myabs(offset_y));
			if (offset_y > 0) {
				b2 = 5;
				offset_y -= n2;
			} else {
